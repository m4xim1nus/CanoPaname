...
- `ui/` — Compose par écran. `ArbresNavHost` avec `startDestination` conditionnelle (`Routes.WELCOME` si `onboardingDone == false`, sinon `Routes.MAP`). Routes : `WELCOME`, `WELCOME_REPLAY`, `MAP`, `MAP_FILTERED`, `ARBORETUM`, `SPECIES`, `PROFILE`, `BADGES`, `REMARQUABLES`, `REMARQUABLE_DETAIL`.
  ...
  - `ui/profile/ProfileScreen.kt` — accessible TopStart de la carte. Stats : 1re capture, # espèces, # remarquables, total captures. Section « Derniers badges » (3 derniers déblocages, `BadgeUnlock(def, displayLabel = def.label, unlockedAt)`) + lien `AllBadgesEntry` → `BadgesScreen`. Section Sauvegarde : boutons Export/Import (SAF), snackbar feedback. Card `HowToPlayEntry` → `Routes.WELCOME_REPLAY`.
  - `ui/badges/` — `BadgesScreen` grille `LazyVerticalGrid` 3 colonnes, spans full-width pour l'en-tête (« X / N débloqués », N = `badgeRepo.catalog.size`) et les titres de section (« Débloqués » / « À débloquer »). `BadgeCard` : icône Outlined (`BadgeDef.icon()`) ou silhouette `Lock` grise si verrouillé, libellé, critère, date d'obtention. `tertiaryContainer` si débloqué, `surfaceVariant` sinon. (Refonte sections par `BadgeCategory` + visuels texte pour les logos d'arrondissement = S5 du cycle Progression — cf. ROADMAP.)
