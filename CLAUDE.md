...
Procédure complète (keystore, clés `local.properties`, `assembleRelease`, `apksigner verify`) dans **`RELEASE.md`**. À savoir ici : sans clé renseignée dans `local.properties`, `assembleRelease` retombe sur le signing debug (fallback conditionnel côté `app/build.gradle.kts`) — utile pour smoke-tester `isMinifyEnabled = true` sans secrets. Le `.gitignore` exclut déjà `*.jks`, `*.keystore` et `local.properties`.
...
- **Capture flow** : `rememberCaptureController` dans `ui/map/CaptureLauncher.kt`. Au tap : permission CAMERA → GPS frais (< 30 s, < 30 m) → URI FileProvider → `TakePicture()` → INSERT Room (check `file.length() > 0`). État pendant (`PendingCapture`) dans le `SavedStateHandle` du `MapViewModel` (survie process death pendant l'intent caméra). La décision « 1re espèce » (`PendingCapture.willCelebrate`) est figée AVANT le launch caméra et pilote à la fois le voile de transition (`MapHost.captureTransitionSk`, rendu par `ArbresNavHost` AU-DESSUS du NavHost — jamais dans `MapScreen`, il fade-rait avec lui pendant la nav) et la nav vers la fiche espèce : la carte n'est jamais visible entre la validation photo et la fiche. Fermer la sheet via `closeDetail()` (sortie de composition), jamais `sheetState.hide()` — la fenêtre dialog de la `ModalBottomSheet` vit au-dessus de tout le contenu de l'Activity, voile compris.
...
- **Pas de service externe au runtime.** La fiche-espèce ne fait pas exception : tout est pré-baké dans `assets/species-info.json` (texte Wikipedia FR + stats Paris) à build-time par `tools/build_dataset.py`. 498 des 937 espèces portent une photo de référence embarquée (`assets/species-photos/` + manifest `species-photos.json`, officielles Ville de Paris + cascade Wikimedia Commons/iNaturalist CC0/PD/CC-BY, cuites à build-time) illustrant le hero de la fiche espèce ; les crédits par image sont générés dans `CREDITS.md` par le build. Toujours aucun appel réseau au runtime.
