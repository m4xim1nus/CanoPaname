...
- `data/` —
  ...
  - **Badges** : `Badge.kt` déclare le catalogue de 8 `BadgeDef(id, label, description, BadgeCategory, tiers?)` en 5 catégories (Découverte, Botanique, Géographie, Remarquables, Démesure). 3 sont **progressifs** (`Marcheur`, `Botaniste`, `Chasseur`) avec une `List<TierDef>` de paliers cumulés ; les 5 autres sont binaires. `BadgeState` est une `sealed class` à deux variants : `Binary(unlockedAt: Long?)` et `Progressive(currentCount: Int, tiers: List<BadgeTier>)`. **Pas de table Room** — tout est dérivé à la volée. `BadgeEvaluator.kt` est une fonction pure `evaluate(captures, arbresById, speciesInfo): List<BadgeState>` ; balayage chronologique unique, accumulateurs (espèces vues, arrondissements, count total, remarquables), `unlockedAt` figé sur la capture qui fait basculer chaque critère (binaire via `unlockBinaryOnce`, ou tier individuel d'un progressif via `unlockProgressive`). Arrondissement parsé via `ArrKey.parseArrKey` (data/ArrKey.kt) qui accepte le format brut OpenData (« …, PARIS 12E ARRDT ») et le format normalisé.
...
- `ui/` — Compose par écran. `ArbresNavHost` avec `startDestination` conditionnelle (`Routes.WELCOME` si `onboardingDone == false`, sinon `Routes.MAP`). Routes : `WELCOME`, `WELCOME_REPLAY`, `MAP`, `MAP_FILTERED`, `ARBORETUM`, `SPECIES`, `PROFILE`, `BADGES`, `REMARQUABLES`, `REMARQUABLE_DETAIL`.
  ...
  - `ui/profile/ProfileScreen.kt` — accessible TopStart de la carte. Stats : 1re capture, # espèces, # remarquables, total captures. Section « Derniers badges » (3 derniers événements de déblocage, chaque palier de progressif compté indépendamment via `BadgeUnlock(def, displayLabel = "{badge} · {tier}", unlockedAt)`) + lien `AllBadgesEntry` → `BadgesScreen`. Section Sauvegarde : boutons Export/Import (SAF), snackbar feedback. Card `HowToPlayEntry` → `Routes.WELCOME_REPLAY`.
  - `ui/badges/` — `BadgesScreen` grille `LazyVerticalGrid` 3 colonnes avec spans full-width pour l'en-tête (« X / 22 paliers débloqués », max = `BadgeCatalog.totalTierCount`) et les titres de section (« Débloqués » / « À débloquer »). `BinaryBadgeCard` : icône Outlined + libellé + critère + date d'obtention si débloqué (silhouette `Lock` grise sinon). `ProgressiveBadgeCard` (full-span 3) : icône, label badge, sous-titre « Prochain palier : {label} » ou « Tous débloqués », score absolu « {currentCount} / {nextThreshold} », barre + jalons cliquables (le slot `onTierClick` est réservé mais pas câblé), date du dernier palier. Couleurs : `tertiaryContainer` au moins 1 palier débloqué / `surfaceVariant` rien débloqué.
...
- **Badges** : ajouter un badge **binaire** = 1 `BadgeDef` (sans `tiers`) dans `Badge.kt` (l'enregistrer dans `BadgeCatalog.ALL`) + 1 appel `unlockBinaryOnce(...)` dans `BadgeEvaluator.evaluate` + 1 entrée dans `BadgeIcons.kt`. Ajouter un badge **progressif** = idem avec `tiers = listOf(TierDef(threshold, label), ...)` + `unitLabel = "captures"|"espèces"|...` (affiché dans la pill score, lève l'ambiguïté entre dimensions comptées) + 1 case dans le `when (def.id)` de l'evaluator (le compteur courant) + 1 appel `unlockProgressive(...)` par capture. Pas de table Room, pas de migration. Le `unlockedAt` (binaire ou par tier) est figé sur la capture déclenchante — un balayage chronologique unique suffit.
...
- Si tu ajoutes un badge : `BadgeDef` dans `data/Badge.kt` (l'enregistrer dans `BadgeCatalog.ALL` ; passer `tiers = ...` pour un progressif) + branche dans `BadgeEvaluator.evaluate(...)` (helper `unlockBinaryOnce` ou `unlockProgressive`, avec un case dans le `when (def.id)` final pour le `currentCount` si progressif) + icône dans `ui/badges/BadgeIcons.kt`. Le compteur du `BadgesScreen` se base sur `BadgeCatalog.totalTierCount` (somme des paliers + binaires) — pas de constante hardcodée.
