...
- `ui/` — Compose par écran. `ArbresNavHost` : `startDestination` **constante** (`Routes.map()`) ; la redirection vers `WELCOME` quand `onboardingDone == false` est portée par un `LaunchedEffect(onboardingDone)`, jamais par `startDestination` (cf. convention « Onboarding + nav » plus bas). Routes : `WELCOME`, `WELCOME_REPLAY`, `MAP`, `MAP_FILTERED`, `ARBORETUM`, `SPECIES`, `PROFILE`, `BADGES`, `REMARQUABLES`, `REMARQUABLE_DETAIL`.
  ...
  - `ui/common/` — `PhotoThumbnail.kt` (décode fichier image local avec downsample paramétrable, partagé Arboretum + fiche-espèce). `SeasonSelector.kt` (pill compacte TopAppBar Arboretum/Remarquables). `ArchiveBanner.kt` (bandeau plein-écran en mode archive read-only). `FrameClock.kt` (`rememberFrameMillis` / `rememberFrameProgress(durationMs, easing)` rampe one-shot 0→1 / `rememberFramePingPong(periodMs, easing)` triangle — boucle `withFrameNanos`, insensible à `MotionDurationScale` contrairement aux API d'animation Compose qui se figent à animation-scale = 0 ; utilisé par le splash, la couronne mini-platanes, le hero `WelcomeScreen`, la célébration 1re capture).
...
- **Géoloc** : `LocationManager` natif uniquement, `play-services-location` retiré. Sur GrapheneOS sans GMS, `LocationEngineDefault` de MapLibre retombe sur `AndroidLocationEngineImpl` (basé sur `LocationManager`). **Bridge MapLibre → `LocationProvider`** : `attachMapLibreLocationBridge(component)` (file-private dans `MapScreen.kt`) attache un `LocationEngineCallback` au `component.locationEngine` ; chaque fix MapLibre nourrit `LocationProvider.feedExternalFix(loc)` qui passe par le même filtre `isBetterFix` que les natural updates. Indispensable au tout 1er run post-onboarding où un `LocationListener` natif registré juste après un grant fresh de permission peut ne recevoir aucun update pendant ~10 s alors que MapLibre reçoit ses fix immédiatement. Cleanup via `removeLocationUpdates(callback)` au `onDispose` du `MapView`. `captureAvailability` est désormais non-bloquant (lecture pure de `currentLocation.value` filtrée sur âge) et le sheet recompute via `remember(openedArbre.id, currentLocation)`. **Cold-start** : `computeInitialCamera` ne fait **jamais** de `getCurrentLocation()` bloquant sur le chemin critique (`LocationProvider.currentLocation.value ?: parisCamera()`, non-`suspend`) — sinon `map.setStyle(...)` attendrait le timeout système (~30 s, GPS froid en intérieur) ; un `LaunchedEffect(mapRef)` recadre la caméra au 1er fix GPS, coupé dès que `userMovedCamera` (geste utilisateur / tap cluster / mode filtré / `pulseArbreId` / caméra mémorisée restaurée).
...
- **Onboarding + nav** : `ArbresNavHost.startDestination` est **une constante** (`Routes.map()`) — surtout pas dérivée d'un `Flow`. `NavHost` mémoïse son graphe sur `(route, startDestination)` ; si `startDestination` changeait (`null → false → true` du round-trip DataStore de `onboardingDone`), le graphe serait reconstruit et `MapScreen` remonté plusieurs fois → une instance transiente jouerait la séquence intro tips **et** appellerait `markSplashIntroSeen()` avant l'instance stable (bug corrigé au cycle Réveil). La redirection vers `WELCOME` se fait via `LaunchedEffect(onboardingDone)` (`navigate(WELCOME)` si `== false`), le splash overlay du `MapScreen` couvre la transition donc pas de flicker. Au tap « Commencer », `popUpTo(WELCOME) { inclusive = true }` pour purger le backstack.
- **Splash tips** : la rotation des tips sous « Réveil des… » est pilotée par `rememberSplashTipText(...)` dans `SplashTipsController.kt`. **Ne pas observer `splashIntroSeen` via `collectAsState`** : on appelle `markSplashIntroSeen()` après la 1re rotation, le re-emit du Flow ferait re-launch le LaunchedEffect rotation et casserait la séquence intro. Le mode (`intro` vs `random`) est lu **une seule fois au mount** via `.first()` et figé dans un `mutableStateOf`. Idem pour le snapshot joueur : `combine(...).first()` une fois, pas de re-collect. Les keys du LaunchedEffect rotation sont volontairement réduites à `(repository, isIntroMode.value, canRotate)` — pas de `playerSnapshot.value` qui resetait l'index. Pour étendre la banque : ajouter des phrases dans `tools/splash-tips-static.json` (catégories `intro` — 10 ids exactement, joués dans l'ordre au tout-1er lancement —, `history`, `popculture`, `player`, `app` ; cette dernière regroupe les tips qui parlent des fonctionnalités de l'app). Les phrases dataset sont générées dynamiquement par `write_splash_tips()` à partir des agrégats CSV ; les placeholders `{xxx}` sont vérifiés au build contre le set runtime `{speciesCount, remarquableCount, daysSinceFirst}` (le `{captureCount}` a été retiré au cycle Réveil — il rendait des tips répétitifs au tout début de partie). Outil de relecture : `tools/build_tips_preview.py` → `docs/tips/index.html` (verdict / commentaire par tip, export copiable).
