...
- ~~[→Polissage] Refonte ciblée de `CLAUDE.md` (Architecture + Conventions trop denses) + nettoyage des références chronologiques (« S9 Lot B », « cycle Catalogue ») dans le code et les `.md` — restaure une prose qui se relit sans contexte interne (user:moi, 2026-05-13) → S4~~ → **livré S4** : `CLAUDE.md` 169 → 149 lignes avec anti-charte intro pour figer la disposition (squelette de packages au lieu d'inventaire file-par-file, 17 invariants 1-liner au lieu de 24 puces denses, détail renvoyé vers les commentaires de tête de fichier déjà en place). 14 fichiers Kotlin + `tools/build_dataset.py` nettoyés (~50 références chronologiques retirées, 0 résidu). Baseline detekt régénérée : 17 → 16 issues (une `LongMethod` disparue par effet de bord). Tests Python verts (87/87), lint OK, detekt OK.
...
- ~~[→Polissage] Hygiène pré-release : (a) réparer les 4 tests JVM pré-existants hérités de S3 (`BadgeEvaluatorTest` × 2 + `SpeciesIndexTest` × 2 — fixtures `SpeciesEntry` à pourvoir d'un `pokedexNumber` pour passer le filtre `isActive`) ; (b) quickfixes detekt catégorie A : `ComplexCondition` `ArbreDetailScreen` (5 nullités chainées → extension `RemarquableInfo.isEmpty`), `MatchingDeclarationName` (extraire `object Routes` dans `Routes.kt`), `ReturnCount MapViewModel.consumePending()` (refactor `?.also`), `LongParameterList ProfileScreen` (13 params → `data class ProgressionState`) ; (c) catégorie B : `LongParameterList ArbreDetailContent` (13 params → `data class ArbreDetailState` + `ArbreDetailActions`). Baseline detekt régénérée. (user:moi + claude:audit-detekt, 2026-05-13) → S5~~ → **livré S5** : 4 tests réparés + 4 quickfixes detekt A + refactor B `ArbreDetailContent`/`DiscoveredContent` via `ArbreDetailState`+`ArbreDetailActions` + 2 refactors sous-radar embarqués (`GenreActions` sur `GenreDetailScreen`, `SpeciesActions` sur `SpeciesDetailScreen`, fichiers dédiés pour matcher `MatchingDeclarationName`). Baseline 16 → 9 issues figées.
- ~~[→Polissage] Incohérence dénominateur genres : Arboretum affiche `X / 203 genres découverts`, Profil affiche `X / 200`. Unifier sur 203 partout (univers `allGenres`, inclut les 3 only-unknown `Genista`/`Vitex`/`Ziziphus`). Effet de bord à fixer au passage : une capture `(G, sp.)` de ces 3 genres ne montait pas le compteur Profil (re-filtré par `genres()` qui ne les contient pas). (user:moi, 2026-05-13) → S6~~ → **livré S6** : `ProfileScreen` migré sur `SpeciesIndex.allGenres()` + `genreHasAnyCapture(g, capturedSpecies)` — mêmes primitives que `ArboretumScreen`, plus court (suppression du `mapNotNull(genreOf)` intermédiaire). KDoc de `genres()` et `allGenres()` clarifiée pour figer la sémantique : `genres()` = chapitres catalogue (200), `allGenres()` = source de vérité progression (203). Bug latent fixé. Aucune migration. `CHANGELOG.md` consolidée par S7 à la clôture.
