...
- [ ] Résidu post-S6 : 27 entrées avec `nv == binôme nu` (count ≤ 2) qui sont en réalité des **synonymes obsolètes ou typos OpenData** (`Olea europea`, `Eriobotrya japonicum` au neutre, `Platanus acerifolia` synonyme de `× hispanica`, `Ulmus parviflora` typo, `Crataegus baccata`/`japonicum` douteux, `Prunus Fleur n. sp.`/`Fruit n. sp.` cultivars génériques, etc.). Solution propre : étendre `SPECIES_FIXUPS` côté `tools/build_dataset.py` pour rebinder les zombies typo vers leur canonique. Pas critique (count très bas, < 3 chacun), mais nettoyage logique (claude:audit-S6, 2026-05-11)
