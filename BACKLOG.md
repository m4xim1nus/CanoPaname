...
- ~~[→Herbier] Upgrade Arboretum (saisonnalité, picto, photos ref) — ligne parapluie~~ **éclatée en 13 sprints S1-S13 le 2026-06-14** (décisions actées : tout d'un coup + cascade photos complète + pas de silhouettes + cellule non capturée garde « ??? »). Plan : `~/.claude/plans/on-attaque-le-cycle-temporal-abelson.md` ; tableau dans `ROADMAP.md`. (user:moi, 2026-05-17)
  ...
  - ~~[→Herbier] **S6** Parsing PDF : champs textuels restants (famille, hauteur chiffrée, envergure, croissance, longévité, descriptions courtes, encarts éditoriaux, services éco). (data, dép. S4)~~ **livré 2026-07-15** (`335a5a3`) — clés `ess` fam/haut/env/croiss/long + iddesc/paris/svc, couverture 194/200 (écorce 193), scores pictos filtrés au niveau span après un NO-GO de vérif corrigé ; « Paysage et cadre de vie » et Statut écartés (décision Max). **Complété le 2026-07-15 (audit visuel sur challenge Max, non committé)** : les 6 fiches « ancien template » sont des exports **rasterisés du template récent** (tous champs visibles, zéro couche texte) → 90 valeurs transcrites à la main (2 agents indépendants, diff 147/147) dans `tools/essence-overrides.json`, fusion fill-only avant `_merge_pdf_extras` → **couverture textuelle 200/200** ; seuls restent les vides véridiques (7 fruct stériles « Aucune/Stérile », écorce Ailanthus absente de la source).
  ...
  - ~~[→Herbier] **S9** Photos officielles 200 (+`Pillow`) : `pymupdf.get_images()` → filtre taille → WebP → `assets/species-photos/` + `species-photos.json` (crédit ODbL Ville de Paris). (data, dép. S4)~~ **livré 2026-07-15 (non committé)** — décision Max : principale + détails (≤ 4/espèce). **155 espèces illustrées / 433 WebP / ~20 Mo** (dont Gymnocladus via fix synonyme, cf. infra) ; `select_photos` pur (recollage fragments, gardes pleine-largeur / y en-tête-pied à deux niveaux / downscale / variance) + `extract_photos` ; manifest `species-photos.json` (meta licences/sources, prêt S10/S11) ; **15 fiches n'ayant QUE le placeholder « Photos à venir » exclues par dHash perceptuel** (8 ré-encodages sur le corpus — le md5 exact en ratait 6 ; 0 photo plutôt qu'une fausse) ; ranking principale par aire de bbox ; préviz `docs/essences/` en chemins relatifs ; 147 tests, build 2× déterministe, zéro churn db/geojson/species-index. Passe de qualité post-livraison sur challenge Max (diagnostic 100 % visuel) : 2 NO-GO de vérif en sprint (placeholders, aplat bleu sk 99) + 3 correctifs post-audit (4 placeholders ré-encodés démasqués, 3 portraits hauts récupérés `_PH_Y_TOP` 70→25, faux détail schéma sk 320 purgé `_PH_Y_TOP_DETAIL`). **Fix matching essence↔catalogue (challenge Max)** : sur 31 fiches non appariées, table `ESSENCE_TAXON_SYNONYMS` dans `_build_essences_index` récupère 2 vrais bugs — Gymnocladus dioicus→dioica (sk 136, 334 arbres, +3 photos) et Sinomalus sieboldii→Malus toringo (sk 338, 37 arbres, +ess seul, fiche sans photo). Les 22 cultivars non appariés sont redondants (espèce nue déjà couverte — `_parse_essence_taxon` retire le cultivar, elle matche déjà) ; 7 irrécupérables (3 espèces à 0 arbre : Aesculus indica, Carya glabra/laciniosa ; 4 hybrides/cultivars sans espèce parente : 3 Ulmus 'résista', X Chitalpa). Reste `[creuser]` : Betula utilis (sk 115) illustré par subsp. albosinensis (écorce rouge) au lieu de subsp. jacquemontii (écorce blanche emblématique) — choix d'illustration, pas un trou.
  - ~~[→Herbier] **S10** Cascade photos 584 : Wikidata P18→iNat, filtre `cc0`/`cc-by` strict → WebP + crédit/licence par image. Surveiller taille APK. (data, dép. S9)~~ **livré 2026-07-15 (non committé)** — décisions Max : espèces identifiées seulement (hors `sp.`), licence stricte CC0/PD/CC-BY (rejet -SA/-NC/-ND), 1 photo/trou, cap 800px q78. **342/649 trous comblés (184 Wikidata P18, 158 iNaturalist)**, +25.1 Mo (dossier `species-photos/` = 45.4 Mo au total S9+S10, 775 WebP). Licences : cc-by 213, pd 79, cc0 50 — **0 licence interdite**, tous `by`/`u` renseignés (attribution CC-BY tracée par image). Nouveau module `tools/species_photos_cascade.py` (fonctions pures testées + réseau + cache 5 niveaux `.species-photos-cache/{p18,commons,inat,images,resolved}` + ledger déterministe) ; `encode_raw_to_webp` partagé extrait de `essence_pdf._encode_photo` (**byte-identité S9 préservée**, 433 WebP intacts) ; manifest `species-photos.json` étendu (meta cc0/pd/cc-by + wikimedia-commons/inaturalist, schéma v1 inchangé) via param `fallback_photos` de `_write_species_photos` (un seul balayage orphelins, union S9∪S10). **Correctif clé** : la recherche iNat `/v1/taxa?q=` n'expose que `default_photo` (souvent réservée) → ajout `fetch_inat_detail` (`/v1/taxa/{id}`) qui seul liste `taxon_photos` CC0/CC-BY (sinon iNat ratait presque tout). 184 tests purs, build 2× déterministe (re-run offline `CANOPANAME_OFFLINE=1` = git propre, zéro churn db/geojson/species-index/species-info), préviz `tools/build_photos_cascade_preview.py` → `docs/photos-cascade/`. Vérif visuelle Max-style sur 12 WebP répartis P18/iNat = tous corrects, 0 doublon, attributions propres.
