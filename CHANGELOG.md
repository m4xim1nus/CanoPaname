...
## [Non publié] — Cycle Boussole

### Corrigé

- **Familier d'arrondissement / Arrondissements complétés** : le critère exigeait la couverture de toutes les *espèces* d'arbres remarquables de l'arr (pivot S1) — divergence avec le compteur `N / M` du Catalogue Remarquables qui, lui, compte des **arbres individuels**. Sur le 19e (12 arbres = 9 espèces uniques), 10 captures suffisaient à débloquer le badge alors que 2 arbres restaient à voir. Le critère bascule sur la **capture de chaque arbre remarquable physique** : `arr-species.json` émet désormais `remarquable_ids: [...idbase...]` à la place de `remarquables: [...sk...]` ; `BadgeEvaluator.evaluateFamilierArr` compare directement des `Set<Long>` d'ids, sans propagation espèce/genre. Effet de bord propre : la sur-permissivité `(Quercus, sp.)` → tout chêne (visible sur les 4e et 15e via `Tamarix sp.` / `Alnus sp.`) disparaît. Aucune migration utilisateur — les badges injustement débloqués se referment au prochain mount du Profil, ré-évalués en sortie d'écran.
- **FAB Recherche / Localiser top de la carte** : le rendu `surfaceContainerHigh` thème-aware donnait deux pastilles gris quasi-noires sur la carte claire OpenFreeMap en thème dark (palette résolue contre le thème app, pas contre la chromie de la carte). Bascule sur du **verre dépoli** `White.copy(alpha = 0.78)` + icône asphalte `Color(0xFF3C4043)`, palette figée hors thème. Élévations strictement à 0 sur tous les états pour éviter le `surfaceColorAtElevation()` qui dessinait un mini-carré clair au centre du FAB par-dessus le container translucide.
- **Sheet Recherche universelle plein écran systématique** : le sheet adoptait la hauteur de son contenu (`OutlinedTextField` + `LazyColumn` bornée à 520 dp), soit ~75 % de l'écran sur un téléphone moderne — perçu comme une « ouverture à 3/4 » avec un re-cadrage quand l'IME montait/redescendait. Le `Column` conteneur passe en `fillMaxSize()`, la `LazyColumn` en `weight(1f)` (suppression du `heightIn(max = 520.dp)`) : le sheet est désormais toujours plein écran, IME ou pas.
- **Source `tools/splash-tips-static.json` désynchronisée** : trois tips citaient encore `930 / 782 espèces` (chiffres pré-`v1.3.2`) et deux tips supprimés en S6 (`player.species_progression`, `player.species_share`) étaient toujours dans la source — chaque régénération `build_dataset.py` ré-introduisait silencieusement la régression. Source réalignée sur `934 / 784` + tips retirés à la source.
