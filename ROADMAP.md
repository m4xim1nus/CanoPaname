...
Numérotation : le S5 effectif correspond à l'ex-« sprint 4bis » dans l'historique git (commit `b0b4e2c`, livré séparément des autres sprints du périmètre `4`). À partir du S6 inclus, la suite est à faire.
...
6. **Cascade `nv` améliorée + strip indéterminée** (data, à faire) : commit fix verify `Non spécifié` (calibration seuil), filtre frTitle != binôme dans la cascade, Wikipedia redirects API (cache `.wikipedia-aliases-cache/`), extension `VERNACULAR_OVERRIDES` top 50 espèces Paris, mapping `GENRE_FR` (~50-100 entrées genre→FR) + suppression du suffixe « (espèce indéterminée) » dans les nv zombies. Verify enrichi (raise sur redondance `(g) e (g) e`). Régénération assets. Audit attendu : ≤ 50 nv = binôme nu (avant : 339).
7. **UI Catalogue 3 modes** (UI + prép data, à faire) : `SegmentedButton` Découverte/Fréquence/Genres dans `ArboretumScreen.kt`, transformations à l'affichage (backend `catalogueOrder` inchangé, ordre Pokédex stable). Mode Genres avec en-têtes de chapitre `GridItemSpan(maxLineSpan)`, count cumulé par genre. Hooks data exposés côté `SpeciesIndex` (`entriesOfGenre`, `genreCount`, `capturedCountInGenre`) en préparation du S8. Tests JVM ajoutés.
8. **Fiches genre dédiées + Mosaïque** (UI clôt le périmètre fonctionnel, à faire) : modèle `GenreEntry` léger dérivé du species-index (pas de nouveau JSON), écran `GenreDetailScreen`, route `Routes.GENRE(genre)`, tap sur en-tête de chapitre du mode Genres + sur titre de fiche `(G, sp.)` pour ouvrir la fiche genre. Couvre les 198 genres avec espèces identifiées + les 3 genres only-unknown (`Genista`, `Vitex`, `Ziziphus`). Badge « Mosaïque de Quercus » (progressif, transféré depuis Variantes) si la complexité reste raisonnable, sinon différé.
9. **Clôture cycle** (à faire) : `SpeciesIndexTest` complet, `./gradlew test lint assembleDebug` vert, balade GrapheneOS pour smoke test (nv propres, 3 modes de tri, fiches genre), entrée `CHANGELOG [1.1.0]`, rotation cycle (compression en « Cycles livrés post-1.0 »), tag `v1.1.0`.
