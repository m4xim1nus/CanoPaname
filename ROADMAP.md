...
5. **S5 — Refonte BadgesScreen pour la nouvelle population**. 58 badges binaires à présenter (10 statiques + 26 Familier de genre + 22 Familier d'arrondissement). Sections par `BadgeCategory` (`DECOUVERTE`, `BOTANIQUE` = rareté + Familier de genre, `GEOGRAPHIE` = Familier d'arrondissement, `REMARQUABLES`, `DEMESURE`). Grille 3 colonnes conservée. Carte Familier : sous-titre dynamique « X / Y espèces » (genre via `SpeciesIndex.genreCount`/`capturedCountInGenre` ; arr via `ArrSpeciesIndex`) pour donner du sens même non débloqué. En-tête « X / N débloqués » avec N = `badgeRepo.catalog.size` (déjà câblé en S3).
   - **Logos** : badges de genre = une icône partagée « bosquet » (`Icons.Outlined.Forest` — un grand arbre + des petits, évoque « tout le genre »). Badges d'arrondissement = le **chiffre romain** (I…XX, cf. `ArrKey.romanNumeral()`) rendu en **texte dans le cercle** ; les 2 bois = juste **« Boulogne »** / **« Vincennes »** en texte. → le slot icône de `BadgesScreen` (`BadgeIconCircle`) doit accepter un visuel **texte** en plus d'un `ImageVector` (placeholders `Forest`/`Place` posés en S3).
