...
### Sprint G — Pokédex remarquables et fiches PDF Ville de Paris
...
- [ ] **Lien fiche PDF Ville de Paris dans la fiche-espèce** — la base OpenData « Fiches Essences du Guide des Essences de Paris » référence ~200 espèces avec un nom de fichier PDF rattaché. Pré-charger en build-time (via `tools/build_dataset.py`) le mapping `(genre, espece) → url_pdf` et l'ajouter à `species-info.json`. Côté UI : afficher dans `SpeciesDetailScreen`, sous la section Wikipedia, un lien « Fiche essence Ville de Paris (PDF) » qui télécharge ou ouvre le PDF dans le navigateur. Pour les ~700 espèces sans entrée correspondante, ne rien afficher (no-op gracieux).
