...
### Sous-groupe F — Polish + GPS (à venir)

Reporté du brief 2026-05-04. À attaquer après E :

- [ ] **GPS first-launch** — au tout premier lancement post-onboarding, `LocationProvider.currentLocation.value` est null pendant ~1-2 s (TTFF) après l'octroi de permission, faisant afficher « Activer le GPS » dans le sheet de capture alors que la permission est OK. Un aller-retour vers le Profil corrige le problème (DisposableEffect re-amorce). Fix possible : observer la permission et le `currentLocation` réactivement dans `MapScreen`, ou seed plus tôt avec `currentOrLastKnown` au mount.
- [ ] **Numéro # catalogue dans la TopAppBar de `SpeciesDetailScreen`** — afficher `#NNN / 907` (`speciesIndex + 1` 1-based / `SpeciesIndex.total`) à côté du nom commun.
- [ ] **Tint icône FAB Arboretum** — icône `MenuBook` actuellement en `LocalContentColor.current`. Tint → `MaterialTheme.arbresColors.feuilleSombre`. Fond du FAB inchangé.
- [ ] **Tint icône FAB loupe (remarquable proche)** — icône `Search` actuellement en `LocalContentColor.current`. Tint → `MaterialTheme.arbresColors.remarquableOrange`. Fond du FAB inchangé.
