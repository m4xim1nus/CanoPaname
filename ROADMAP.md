...
- [ ] **Climax du moment de capture** — le tap déclenche aujourd'hui une INSERT Room + une recoloration de point, sans cérémonie. Pipeline visuel proposé : ripple subtil depuis le doigt (Compose natif) → silhouette grise du point qui se colorie *par le bas* (Canvas + clip animé, ~200 ms) → si 1re d'une espèce, nom binomial en Fraunces qui apparaît au-dessus, hold 1 s, fade. Bloom contemplatif, pas explosion façon Pokémon GO. C'est *le* climax émotionnel de l'app.
