...
2. ✅ **S2 — Suppression des badges progressifs** (livré). Démantèlement de `BadgeState.Progressive`, `TierDef`, `BadgeTier`, `unlockProgressive`, `ProgressiveBadgeCard` & co. `BadgeState` aplati en `data class(def, unlockedAt)`. Retrait du catalogue : `MARCHEUR`, `BOTANISTE`, `CHASSEUR`, `MOSAIQUE_QUERCUS` (l'identité Quercus revient en S3 en binaire) — `BadgeCatalog.ALL` retombe à 5 binaires. `BadgeEvaluator.evaluate` ne fait plus que des `unlockBinaryOnce` ; le param `speciesIndex` (sans consommateur restant) est retiré, idem côté `BadgeRepository`. Pas de migration Room (badges dérivés). Le compteur de `BadgesScreen` est câblé sur `BadgeCatalog.ALL.size` (wording « X / N badges » final en S5).
