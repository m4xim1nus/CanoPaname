...
- **S5 — Hygiène pré-release** : (a) réparation des 4 tests pré-existants hérités de S3 (`BadgeEvaluatorTest` × 2 + `SpeciesIndexTest` × 2 — fixtures `SpeciesEntry` qui n'ont pas suivi l'introduction du filtre `isActive` : ajouter `pokedexNumber` aux entries de test). (b) Quickfixes detekt catégorie A : `ComplexCondition` `ArbreDetailScreen` → extension `RemarquableInfo.isEmpty`, `MatchingDeclarationName` → extraire `object Routes` dans `Routes.kt`, `ReturnCount MapViewModel.consumePending()` → `?.also`, `LongParameterList ProfileScreen` (13 params) → `data class ProgressionState`. (c) Catégorie B : `LongParameterList ArbreDetailContent` (13 params) → `data class ArbreDetailState` + `ArbreDetailActions`. Baseline detekt régénérée. Aucun changement de comportement.
