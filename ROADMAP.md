...
### Sprint H — Profil et premiers badges ✅ 2026-04-29
...
- [x] **Bouton Profil en TopStart de la carte** ✅ 2026-04-29 — `FloatingActionButton` icône `Person` en haut-gauche, désactivé en mode `MAP_FILTERED` (où la zone est occupée par le `FilterBanner`). Cohérent avec la grammaire FAB existante (GPS BottomEnd, Arboretum/★ TopEnd, 🔍 Loupe BottomStart).
- [x] **Écran Profil minimal** ✅ 2026-04-29 — route `Routes.PROFILE`, `ProfileScreen` Compose. Stats : (1) « il y a N jours » depuis la 1re capture (calculé via `MIN(timestamp)` sur `capture` exposé en Flow par `CaptureDao.firstCaptureTimestamp()`), (2) # espèces distinctes capturées (`capturedSpeciesIndices.size`), (3) # arbres remarquables distincts capturés (`capturedRemarquableIds.size`), bonus (4) captures totales (`COUNT(*)`). Quand 0 capture, affichage gracieux « Aucune capture / — ». Stats globales toutes saisons confondues — la bascule saisonnière arrivera en Sprint I.
- [x] **Liste de badges minimale** ✅ 2026-04-29 — `data/Badge.kt` : `BadgeDef` (id, label, description) + `BadgeState` (def, unlockedAt) + singleton `BadgeCatalog.ALL`. Une seule entrée à ce stade : `FIRST_CAPTURE`, débloqué quand `firstCaptureTimestamp != null`, `unlockedAt = firstCaptureTimestamp`. Affichage : grille 3 colonnes dans `ProfileScreen`, cellules débloquées avec libellé + date courte, cellules verrouillées en silhouette `Lock` + « ??? ». Pas de table Room : le déblocage se calcule à partir des captures existantes — ajouter un badge en Phase 4 = ajouter une `BadgeDef` + une branche dans le `when` qui calcule `unlockedAt`.
